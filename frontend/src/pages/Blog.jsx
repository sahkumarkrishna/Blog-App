    ...
    const getAllPublishedBlogs = async () => {
      try {
        const res = await axios.get(
          `https://blog-app-xqmy.onrender.com/api/v1/blog/get-published-blogs`,
        ...
        if (res.data.success && res.data.blogs) {
          ...
          dispatch(setBlog(blogJson)); // fallback if response is not valid
        }
      } catch (error) {
  ...
  return (
    <div className="pt-16">
      ...
      <div className="max-w-6xl mx-auto grid gap-10 grid-cols-1 md:grid-cols-3 py-10 px-4 md:px-0">
        {blog?.length > 0 ? (
          blog.map((item, index) => (
            <BlogCard blog={item} key={item._id || item.id || index} />
          ))
        ) : (
